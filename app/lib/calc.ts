...
export function mfToApr(mf: number): number {
  return mf * 2400;
}

export function aprToMf(apr: number): number {
  return apr / 2400;
}
...
export function calcDeal(deal: Deal, pricing: Pricing, trades: Trade[]): CalcResult {
  ...
  if (deal.type === 'finance') {
    ...
    return { payments, footerLabel: 'Amt. Financed', footerVal: activePrimary, tradeEquity };
  }

  // Lease
  // Ohio taxes monthly lease payments rather than the cap cost (ORC 5739.01).
  // Trade allowance credit applicability to lease tax is deferred pending
  // further research into Ohio lease tax law.
  const taxRate = Number(pricing.taxRate) || 6.75;
  const calcLeasePayment = (downAmount: number | null): { payment: number; primary: number } => {
    const down        = Number(downAmount) || 0;
  ...
  const payments = (deal.downPayments || []).map(dp => ({
    amount: dp.amount,
    computed: calcLeasePayment(dp.amount),
  }));
  const activePrimary = payments[deal.activeDP]?.computed.primary ?? 0;
  return { payments, footerLabel: 'Cap Cost', footerVal: activePrimary, tradeEquity };
}
